...
def A_coef(X):
    return u_darcyFlow_Houman(X, 1.0)

def u_darcyFlow_Houman(X, B):
    return np.exp(np.sin(B * np.sum(np.cos(X), axis=-1)))[..., None]

def dAxi(i_dim, X):
    return duxi(i_dim, X, 1.0)

def duxi(i_dim, X, B):
    return -B * u_darcyFlow_Houman(X, B) * (np.cos(B * np.sum(np.cos(X), axis=-1))[..., None]) * np.sin(X[:, i_dim:(i_dim+1)])