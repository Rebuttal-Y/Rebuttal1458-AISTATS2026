...
store_path=$SCRIPT_DIR"/result/Data_DarcyFlow6D_CP_ALS_PF"
store=$store_path
if [ ! -d $store ]; then
    mkdir -p $store
fi
                    ...
                    do
                        ...
                        do
                            ...
                            do
                                ...
                                python SolveDarcyFloW6D_CP.py --epochs=1000000 --early-stop=5000 --log-store-path=$store_directory"/config"$count \
                                 ...
                                 --rank=${rank[$rdx]} --n-xind ${n_xind[$nxdx]} ${n_xind[$nxdx]} ${n_xind[$nxdx]} ${n_xind[$nxdx]} ${n_xind[$nxdx]} ${n_xind[$nxdx]} \
                                 --log-interval=100 --lam1=${lam1[$l1dx]} --lam2=${lam2[$l2dx]} --test-dataset-load-path=$dataset_path"/DarcyFlow_Houman_Test1000000.npz" \
                                 --dataset-load-path=$dataset_path"/Beta"$(round ${beta[$bdx]} 0)"/DarcyFlow_HoumanC"${n_train_col[$nxdx]}"_B"$((((${n_train_bound[$nxdx]} + 6 * 2 - 1)) / ((6 * 2)) * ((6 * 2))))"_Trace.npz" \
                                 --analysis
                                count=$(( count + 1 ))
                            done
                        done
