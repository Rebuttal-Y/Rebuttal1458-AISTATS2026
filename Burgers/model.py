...
class GP_ALS(object):
    ...
    def __init__(self, args, data_gen):
        ...
        self.X_inducing = [np.linspace(self.data_gen.x_range[i][0], self.data_gen.x_range[i][-1], num=self.args.n_xind[i]) \
    ...
    def _prepare(self):
        ...
        #u_t
        KCrossCol_Ut = [cross_kernel(self.X_c[:, 0], self.X_inducing[0], self.ker_func[0].kappa, self.ker_params[0]),
                         cross_kernel(self.X_c[:, 1], self.X_inducing[1], self.ker_func[1].D_x1_kappa, self.ker_params[1])] #Nc x M 
        KInvCrossCol_Ut = [jnp.linalg.solve(K[j], KCrossCol_Ut[j].T) for j in range(self.args.n_order)] 
        ...
        self.KInvCrossCol_Ut = KInvCrossCol_Ut
    ...
    def update(self):
        ...
        for j in range(2):
            ...
            #u_x
            A1 = self.KInvCrossCol_Ux[j].T #Nc x M
            B1 = self.KInvCrossCol_Ux[o].T @ self.H[o] #Nc x R
            coef1 = jnp.sum((A1 @ self.H[j]) * B1, 1) #Nc
            #u_xx
            A2 = self.KInvCrossCol_Uxx[j].T #Nc x M
            B2 = self.KInvCrossCol_Uxx[o].T @ self.H[o] #Nc x R 
            #u_t
            A3 = self.KInvCrossCol_Ut[j].T #Nc x M
            B3 = self.KInvCrossCol_Ut[o].T @ self.H[o] #Nc x R
            ...
            while idx_start < n_total:
                ...
                if self.args.Newton_M:
                    # Phi2 = jnp.concatenate((Phi2, jnp.einsum('ij,ik->ijk', A3[idx_start:idx_end, ...], B3[idx_start:idx_end, ...]) + \
                    #     jnp.einsum('ij,ik->ijk', A1[idx_start:idx_end, ...], B1[idx_start:idx_end, ...])*coef0[idx_start:idx_end, None, None] + \
                    #         jnp.einsum('ij,ik->ijk', A0[idx_start:idx_end, ...], B0[idx_start:idx_end, ...])*coef1[idx_start:idx_end, None, None] - \
                    #             coef0[idx_start:idx_end, None, None] * coef1[idx_start:idx_end, None, None] - self.args.nu*jnp.einsum('ij,ik->ijk', A2[idx_start:idx_end, ...], B2[idx_start:idx_end, ...])), axis=0)

                    Phi2 = jnp.concatenate((Phi2, jnp.einsum('ij,ik->ijk', A3[idx_start:idx_end, ...], B3[idx_start:idx_end, ...]) + \
                        jnp.einsum('ij,ik->ijk', A1[idx_start:idx_end, ...], B1[idx_start:idx_end, ...])*coef0[idx_start:idx_end, None, None] + \
                            jnp.einsum('ij,ik->ijk', A0[idx_start:idx_end, ...], B0[idx_start:idx_end, ...])*coef1[idx_start:idx_end, None, None] - self.args.nu*jnp.einsum('ij,ik->ijk', A2[idx_start:idx_end, ...], B2[idx_start:idx_end, ...])), axis=0)
                else:
                    Phi2 = jnp.concatenate((Phi2, jnp.einsum('ij,ik->ijk', A3[idx_start:idx_end, ...], B3[idx_start:idx_end, ...]) + \
                        jnp.einsum('ij,ik->ijk', A1[idx_start:idx_end, ...], B1[idx_start:idx_end, ...])*coef0[idx_start:idx_end, None, None] - \
                            self.args.nu*jnp.einsum('ij,ik->ijk', A2[idx_start:idx_end, ...], B2[idx_start:idx_end, ...])), axis=0)
            ...
            # eta = Phi1.T@self.y_b # (M*R)
            if self.args.Newton_M:
                eta = Phi1.T@self.y_b + self.args.lam2/self.args.lam1*Phi2.T@(coef0 * coef1) # (M*R)
            else:
                eta = Phi1.T@self.y_b# (M*R)
    ...
    def get_loss(self):
        ...
        ut = grad(self.solution_approx, 1)
        ...
        ut_pred = vmap(ut, (0, 0))(self.X_c[:, 0], self.X_c[:, 1]) #N_c
        g_pred = ut_pred + u_pred*ux_pred - self.args.nu*uxx_pred #N_c
        residual = jnp.square(g_pred).mean() #collocation loss
    ...
    def get_residual(self):
        ...
        Ux = jnp.sum(Ux, 1)
        Ut = (self.KInvCrossCol_Ut[0].T@self.H[0])*(self.KInvCrossCol_Ut[1].T@self.H[1])
        Ut = jnp.sum(Ut, 1)
        U = (self.KInvCrossCol_U[0].T@self.H[0])*(self.KInvCrossCol_U[1].T@self.H[1]) #Nc x R
        U = jnp.sum(U, 1) #Nc
        g_pred = Ut + U * Ux - self.args.nu * Uxx #Nc
        residual = jnp.square(g_pred).mean()
        return residual 