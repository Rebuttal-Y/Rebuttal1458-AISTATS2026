...
def do_analysis(model, data_gen, change, iter, err):
    pred_tr = model.pred(jnp.array(data_gen.x_train_boundary)).reshape(-1)
    ground_truth = jnp.array(data_gen.y_train_boundary).reshape(-1)
...
def model_run(model, data_gen):
    ...
    while start_epoch < model.args.epochs:
        ...
        change = sum([jnp.linalg.norm(model.H[i] - old_H[i])/jnp.linalg.norm(old_H[i]) for i in range(model.args.n_order)])
