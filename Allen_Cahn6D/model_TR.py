...
class GP_ALS_6D(object):
    ...
    def update(self):
        ...
        for j in range(self.args.n_order):
            ...
            left_uvv = np.tile(np.eye(self.args.rank[0])[None, ...], (n_col, 1, 1))
            left_uss = np.tile(np.eye(self.args.rank[0])[None, ...], (n_col, 1, 1))
            for i in range(j):
                ...
                left_uvv = jnp.einsum("bac,cbd->bad", left_uvv, self.KInvCrossCol_Uvv[i].T @ self.H[i])
                left_uss = jnp.einsum("bac,cbd->bad", left_uss, self.KInvCrossCol_Uss[i].T @ self.H[i])
            ...
            right_uvv = np.tile(np.eye(self.args.rank[0])[..., None], (1, 1, n_col))
            right_uss = np.tile(np.eye(self.args.rank[0])[..., None], (1, 1, n_col))
            for k in reversed(range(j+1, self.args.n_order)):
                ...
                right_uvv = jnp.einsum("abc,cdb->adb", self.KInvCrossCol_Uvv[k].T @ self.H[k], right_uvv)
                right_uss = jnp.einsum("abc,cdb->adb", self.KInvCrossCol_Uss[k].T @ self.H[k], right_uss)
            ...
            #uvv
            A5 = self.KInvCrossCol_Uvv[j].T
            B5 = jnp.transpose(jnp.einsum("acb,bcd->bad", right_uvv, left_uvv), (0, 2, 1)).reshape(n_col, -1)
            #uss
            A6 = self.KInvCrossCol_Uss[j].T
            B6 = jnp.transpose(jnp.einsum("acb,bcd->bad", right_uss, left_uss), (0, 2, 1)).reshape(n_col, -1)
    ...
    def get_residual(self):
        ...
        Uvv = self.KInvCrossCol_Uvv[0].T@self.H[0]
        Uss = self.KInvCrossCol_Uss[0].T@self.H[0]
        ...
        for i in range(1, self.args.n_order):
            ...
            Uvv = jnp.einsum("abc,cbd->abd", Uvv, self.KInvCrossCol_Uvv[i].T@self.H[i])
            Uss = jnp.einsum("abc,cbd->abd", Uss, self.KInvCrossCol_Uss[i].T@self.H[i])
        ...
        Uvv = jnp.einsum("ibi->b", Uvv)
        Uss = jnp.einsum("ibi->b", Uss)
