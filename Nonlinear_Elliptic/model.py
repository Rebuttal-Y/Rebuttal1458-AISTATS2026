...
class GP_ALS(object):
    ...
    def update(self):
        ...
        for j in range(2):
                ...
                if self.args.Newton_M:
                    Phi2 = jnp.concatenate((Phi2, -jnp.einsum('ij,ik->ijk', A1[idx_start:idx_end, ...], B1[idx_start:idx_end, ...]) - \
                        jnp.einsum('ij,ik->ijk', A2[idx_start:idx_end, ...], B2[idx_start:idx_end, ...]) + \
                            3.0*coef0[idx_start:idx_end, None, None]*coef0[idx_start:idx_end, None, None]*\
                                jnp.einsum('ij,ik->ijk', A0[idx_start:idx_end, ...], B0[idx_start:idx_end, ...])), axis=0)
                else:
                    Phi2 = jnp.concatenate((Phi2, -jnp.einsum('ij,ik->ijk', A1[idx_start:idx_end, ...], B1[idx_start:idx_end, ...]) - \
                    jnp.einsum('ij,ik->ijk', A2[idx_start:idx_end, ...], B2[idx_start:idx_end, ...]) + jnp.einsum('ij,ik->ijk', A0[idx_start:idx_end, ...], \
                        B0[idx_start:idx_end, ...])*coef0[idx_start:idx_end, None, None]*coef0[idx_start:idx_end, None, None]), axis=0)
                idx_start = idx_end
            ...
            if self.args.Newton_M:
                eta = self.args.lam2/self.args.lam1*Phi2.T@(self.y_c + 2.0*coef0*coef0*coef0)# (M*R) 
            else:
    ...
    def get_loss(self):
        ...
        g_pred = - uxx_pred - uyy_pred + u_pred*u_pred*u_pred #N_c
